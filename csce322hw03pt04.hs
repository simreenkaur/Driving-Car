...
main = do
 ...
 print "Result"
 printMap (manyVehiclesManyTurns map turns)

-- YOUR CODE SHOULD COME AFTER THIS POINT
manyVehiclesManyTurns :: [[Char]] -> [Char] -> [[Char]]
manyVehiclesManyTurns map turns = manyVehiclesOneTurnHelper map turns 1

manyVehiclesOneTurn :: [[Char]] -> [Char] -> [[Char]]
manyVehiclesOneTurn map turns = manyVehiclesOneTurnHelper map turns 1

manyVehiclesOneTurnHelper :: [[Char]] -> [Char] -> Int -> [[Char]]
manyVehiclesOneTurnHelper map (head_4 : tail_4) vehicle
                | (vehicle > (find_vehicles map)) = manyVehiclesOneTurnHelper updated_map (head_4 : tail_4) 1
                | ((head_4 : tail_4) == []) = map
                | (((head_4 : tail_4) /= []) && (tail_4 == [])) = oneVehicleOneTurn map head_4 vehicle_converted
                | otherwise = manyVehiclesOneTurnHelper updated_map tail_4 updated_vehicle
                where updated_map = oneVehicleOneTurn map head_4 vehicle_converted
                      vehicle_converted = (convert vehicle)
                      updated_vehicle = vehicle + 1
...
find_vehicles :: [[Char]] -> Int
find_vehicles [] = 0
find_vehicles (row : rows) = (find_vehicles_row row) + (find_vehicles rows)

find_vehicles_row :: [Char] -> Int
find_vehicles_row [] = 0
find_vehicles_row (h : t)
               | ((isDigit h ) == False) = 0 + (find_vehicles_row t)
               | ((isDigit h ) == True ) = 1 + (find_vehicles_row t)

oneVehicleOneTurn :: [[Char]] -> Char -> Char ->[[Char]]
oneVehicleOneTurn map turn vehicle
